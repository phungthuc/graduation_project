...
namespace TheTunnel
{
    public class Custom_UIController : UIController
    {
        [SerializeField]
        ...
        {
            base.OnEnable();
            UIEvents.onConstructStarted += turretInventory.Open;

            UIEvents.countDownStarted += OnCountDownStarted;
            UIEvents.countDownUpdated += countDownText.SetText;
            UIEvents.countDownFinished += OnCountDownFinished;
        }

        protected override void OnDisable()
        {
            base.OnDisable();
            UIEvents.onConstructStarted = null;

            UIEvents.countDownStarted = null;
            UIEvents.countDownUpdated = null;
            UIEvents.countDownFinished = null;
        }
...
#if UNITY_EDITOR
    [System.Serializable]
    [CustomEditor(typeof(Custom_UIController))]
    public class UIControllerEditor : Editor
    {
        private string[] tabs = { "Health", "Interaction", "TurretInventory", "Attachments", "Weapon", "Dashing", "Experience", "Others", "UI Events" };
        private int currentTab = 0;

        override public void OnInspectorGUI()
        {
            serializedObject.Update();
            Custom_UIController myScript = target as Custom_UIController;
            ...
            if (currentTab >= 0 || currentTab < tabs.Length)
            {
                switch (tabs[currentTab])
                {
                    ...
                    case "UI Events":
                        ...
                        break;
                    case "TurretInventory":
                        EditorGUILayout.LabelField("TURRET INVENTORY", EditorStyles.boldLabel);
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("turretInventory"));
                        break;
                }
            }
